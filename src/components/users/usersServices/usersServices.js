...
class UsersServices {
  ...
  resetPassByEmail = async (email, res, req) => {
    ...
    `;
      const attachments = [
        {
          filename: 'crisan_iso-06.png',
          path: path.join(__dirname, '../../../uploads/mail/crisan_iso-06.png'),
        },
      ];
      const emailPayload = {
        from: 'optica.crisan@gmail.com',
        to: user.email,
        subject: 'Óptica crisan - Reestablecimiento de contraseña',
        html: emailContent,
        attachments,
      };
      ...
      if (inactiveUsers.length > 0) {
        ...
        `;
          ...
          const emailPayload = {
            from: 'optica.crisan@gmail.com',
            to: user.email,
            subject: 'Óptica Crisan - Cuenta eliminada por inactividad',
            html: emailContent,
            attachments,
          };
          try {
